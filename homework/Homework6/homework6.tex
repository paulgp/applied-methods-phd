...
\item \textbf{Diff-in-diff:} We first consider a simple diff-in-diff
  ...
  \item Finally, use the dummy variable \texttt{elas\_bin} (which is
    ...
    on log house prices.
  \end{enumerate}
% \item \textbf{Synthetic Control/DiD:} We now implement the same analysis as the
%   previous section, but using the synthetic methods approach.
%   \begin{enumerate}
%   \item Using the \texttt{synthdid} package, estimate the effect of
%     \texttt{elas\_bin} on house prices, defining the treatment period
%     as starting in 2002. What is the impact of \texttt{elas\_bin} on house prices? 
%   \end{enumerate}
