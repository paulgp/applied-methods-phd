...
\begin{columns}[T] % align columns
  \begin{column}{.7\textwidth}
    ...
    \item \textbf{Important}: do not lose sight of the fact that these
      should be estimates that inform our economic model (fully formalized or not)
    ...
    \item The PO framework's key insight was considering the sets of
      potential outcomes for the same individuals as a form of missing
      data. Note that these are effectively  equivalent:
      \begin{align*}
        Y_{i} &= D_{i}Y_{i}(1) + (1-D_{i})Y_{i}(0)\\
