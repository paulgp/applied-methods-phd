\message{ !name(regression_4.tex)}\documentclass[notes,11pt, aspectratio=169]{beamer}
...
\message{ !name(regression_4.tex) !offset(148) }
...
\begin{frame}
  \begin{wideitemize}
    \item An aside on Pointwise vs. Uniform convergence
  \end{wideitemize}
\end{frame}

\begin{frame}{Back to lasso's amazing oracle property}
  \begin{columns}[T] % align columns
    \begin{column}{.8\textwidth}
      \begin{wideitemize}
      \item Leeb and Potscher (2008) say wait hold on
      \end{wideitemize}
    \end{column}%
  \hfill%
  ...
  \begin{columns}[T] % align columns
    \begin{column}{.8\textwidth}
      ...
      \item Consider an estimator $\hat{\theta}_{n}$ for $\theta$. Now we construct our new estimator,
        \begin{itemize}
        \item $\hat{theta}_{n,hodge} = 0$ if $|\hat{theta}_{n
\message{ !name(regression_4.tex) !offset(210) }
