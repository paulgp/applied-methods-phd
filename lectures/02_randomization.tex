...
\begin{frame}{Growth in ``Research Design'' in NBER Workin Papers}
\includegraphics[width=\linewidth]{images/research_design_nberwp.png}
\end{frame}
...
\begin{columns}[T] % align columns
  ...
  \item Let $\Omega$ denote that space of possible values that
      ...
      experiment one runs.
      \begin{itemize}
      \item If each individual has a fair coin flipped on whether they are
      ...
      \item Other ways to consider randomly assigning individuals
        \begin{itemize}
        \item Random draws from an urn (to ensure an exact number treated) -- this is known as a \emph{completely randomized experiment} -- this is commonly used and convenient
        \item Clustering individuals on characteristics (or location) -- this is known as a \emph{stratified randomized experiment}
        \end{itemize}
      \end{itemize}
    \end{wideitemize}
  \end{column}%
