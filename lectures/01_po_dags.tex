...
\begin{column}{.8\textwidth}
  \begin{wideitemize}
    \item  Two variables:
    \begin{itemize}
    \item $Y \in \{0, 1\}$: whether a person will get measles over their lifetime
    \item $D \in \{0, 1\}$: whether a person gets a vaccine as a child
    \end{itemize}
...
\begin{column}{.85\textwidth}
  ...
  \item We will start with the \underline{Average Treatment Effect}:
    ...
    \end{itemize}
  \item \underline{Average Treatment Effect on the Treated}:
    \begin{align*}
    \tau_{ATT} = \mathbb{E}(\tau_{i} | D_{i} = 1) &= \mathbb{E}(Y_{i}(1) - Y_{i}(0) | D_{i} = 1) \\ 
     &= \mathbb{E}(Y_{i}(1) | D_{i} = 1) - \mathbb{E}(Y_{i}(0) | D_{i} = 1)
    \end{align*}\vspace{-10pt}
    \begin{itemize}
    \item Estimated effect for individuals who \emph{received} the treatment.
    \item Note that $\mathbb{E}(Y_{i}(1) | D_{i} = 1)$ is  observed data!
    \end{itemize}
  \item \underline{Conditional Average Treatment Effect}: For a characteristic $X_{i}$,
   \begin{equation*}
   \tau_{CATE}(x) = \mathbb{E}(\tau_{i} | X_{i} = x) = \mathbb{E}(Y_{i}(1) - Y_{i}(0) |  X_{i} = x)
   \end{equation*}
  \end{wideitemize}
\end{column}%
\hfill%
...
\begin{itemize}
  \item Why would something not be identified if we only observe $(Y_{i}, D_{i})$?
    ...
    \item One approach: make an assumption on the relationship between $D_{i}$ and
      $(Y_{i}(1), Y_{i}(0))$
      \begin{itemize}
        \item This is known as a \emph{design}-based approach (more later)
      \end{itemize}
    \end{itemize}
\end{itemize}
\end{frame}
...
\begin{columns}[T] % align columns
  \hspace{5pt} \begin{column}{.8\textwidth} \underline{\textbf{Strong
    ...
    \item N.B. The term ``strong ignorability'' is much more precise than exogeneous
      ...
      \item You \emph{might} even say $D_{i}$ is exogeneous.
      \end{itemize}
    \end{itemize}
