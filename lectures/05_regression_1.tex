...
\begin{frame}{Doing this in practice}
  ...
  \item What are the options for estimating $\hat{\sigma}^{2}(x)$?  
    \begin{align*}
      u_{j} &= Y_{j} - X_{j}\beta, h = \mathbf{X}(\mathbf{X}'\mathbf{X})^{-1}\mathbf{X}' & \\
      \hat{\sigma}^{2} &= \frac{1}{n-k} \sum_{j} u_{j}^{2} \qquad \qquad&  \text{(robust)}\\
      \hat{\sigma}^{2} &=  \frac{1}{n}\sum_{j} u_{j}^{2}(1-h_{jj}) \qquad \qquad& \text{(HC2)}\\
      \hat{\sigma}^{2} &=  \frac{1}{n}\sum_{j} u_{j}^{2}(1-h_{jj})^{2} \qquad \qquad& \text{(HC3)}\\
    \end{align*}
