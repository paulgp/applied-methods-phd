...
\textbf{Email:} &  \href{mailto:paul.goldsmith-pinkham@yale.edu}{paul.goldsmith-pinkham@yale.edu} & & & \textbf{Location:} & Evans Hall 4210
\end{tabular}
\end{minipage}
\rule{6.5in}{0.4pt}
\end{center}
...
\item \url{https://yale.instructure.com/courses/104052} [Yale students only]
\end{enumerate}
...
asked to analyze data sets.. Solutions will be handed out written in R. Since there will be a fair number of problem sets, and in order to allow me to post the solutions quickly on the webpage for the course, I will not accept
...
to work on it. Since there are a very large number of students in the class, I will not be providing individual feedback on the problem sets, but I am happy to discuss them in office hours.

In order to access these problem sets, you will need to have a Github account, as I will be using the Github classroom feature to distribute the problem sets. You can work together on the problem sets and discuss them with
...
specify when canned packages are appropriate. In other words, when the problem is about estimating a regression, and it is not specified otherwise, I am not looking for the results of \texttt{lm( y $\sim$ x)}. Rather, I expect you to construct two matrices and calculate the estimates using this. I also expect you to attempt to maintain good coding practices while doing so -- this will
likely be challenging for those of you who are inexperienced at
programming, so please plan according. See
...
You are also very welcome (and encouraged) to use any and all large language model help when working on the problem sets! However, an important caveat to this is that the purpose of these problem sets is for you to learn -- using the tool to finish the task is fine, but not when it replaces any struggle or effort on your part.  There will be a particular section on the problem set that is an AI-free-zone, where I will request that you do not use LLMs to give your responses.
...
Recommended reading is starred and in bold.
\begin{enumerate}
\item \underline{Causality, Statistics, and Economics}
  \begin{enumerate}
  \item Class 1: Potential Outcomes and Directed Acyclic Graphs
    \begin{itemize}
              \item \textbf{``Potential Outcome and Directed Acyclic Graph Approaches to Causality: Relevance for Empirical Practice in Economics'', Imbens (2020)}$^{**}$
    ...
    \item ``Models, Measurement, and the Language of Empirical Economics'', Haile \url{https://sites.google.com/view/philhaile/home/teaching
    }
  ...
  \item Class 2: Research Design, Randomization, and Model- vs. Design-Based Inference
    \begin{itemize}
      \item \textbf{``The Credibility Revolution in Empirical Economics: How Better Research Design is Taking the Con out of Econometrics'', Angrist and Pischke (2010)}$^{**}$
  ...
  \item Class 3: Propensity Scores
    \begin{itemize}
      \item \textbf{LaLonde (1986) after Nearly Four Decades:
Lessons Learned, Imbens and Xu (2024)}$^{**}$
  ...
  \item Class 4: Interference, Spillovers and Dynamics
    \begin{itemize}
      \item \textbf{``Toward Causal Inference With Interference'', Hudgens and Halloran (2008)}$^{**}$
...
\item \underline{Linear Regression}
  ...
  \item Class 5: Inference
    \begin{itemize}
      \item  \textbf{``When Should You Adjust Standard Errors for Clustering?'' Abadie et al. (2022)}$^{**}$
  ...
  \item Class 6: Semiparametrics and Visualization
    \begin{itemize}
    \item \textbf{``On Binscatter'' Cattaneo et al. (2019)}$^{**}$
  ...
  \item Class 7: Quantile Regression
    \begin{itemize}
    \item \textbf{``Quantile Regression'' Koenker and Hallock (2001)}$^{**}$
    \item ``Quantile Regression: 40 Years On'' Koenker (2017)
    \end{itemize}
  \item Class 8: Penalized linear regression
    \begin{itemize}
      \item \textbf{``Machine learning: an applied econometric approach'' Mullainathan and Spiess (2017)}
...
\item \underline{Likelihood Methods}
  ...
  \item Class 9: Binary Discrete Choice, GLM and Computational Methods
    \begin{itemize}
      \item \textbf{``Logs with zeros? Some problems and solutions'' Chen and Roth (2024)}
  ...
  \item Class 10: Multiple Discrete Choices
    \begin{itemize}
      \item \textbf{``Economic Choices''  (2001, American Economic Review) Daniel Mcfadden }
  ...
  \item Class 11: Duration models
    ...
    \item \textbf{``Duration Dependence and Labor Market Conditions: Evidence from a Field Experiment'', Kroft, Lange and Notowidigdo (2013)}
  ...
  \item Class 12: Hierarchical modeling + Bayesian Shrinkage
    ...
    \item \textbf{``Understanding the average impact of microcredit expansions: A bayesian hierarchical analysis of seven randomized experiments'' Meager (2019)}
...
\item  \underline{Canonical Research Designs}
  \begin{enumerate}
  \item Class 13: Difference-in-differences (Part I): single timing, multiple time periods
    \begin{itemize}
    \item \textbf{``How Much Should We Trust  Difference-In-Differences Estimates?'' Bertrand, Duflo, and Mullanathan (2004)}
    \item ``Minimum Wages and Employment: A Case Study of the Fast-Food lndustry in New Jersey and Pennsylvania'' Card and Krueger (1994)
    ...
    \item ``Pretest with caution: Event-study estimates after testing for parallel trends'' Roth (2022)
    \item ``Pre-event Trends in the Panel Event-study Design'' Freyaldenhoven, Hansen and Shapiro (2019)
    \item ``(Visualizing) Plausible Treatment Effect Paths'' Freyaldenhoven and Hansen (2026)
    \item ``Placebo inference on treatment effects when the number of clusters is small'' Hagemann (2019)
    \item ``Permutation inference with a finite number of heterogeneous clusters'' Hagemann (2025)
    \item ``Inference with a single treated cluster'' Hagemann (2025)
    \end{itemize}
  \item Class 14: Difference-in-differences (Part II): Event Studies, Synthetic Control + Synthetic DinD
    \begin{itemize}
    \item \textbf{``Difference-in-differences with multiple time periods'', Callaway and Santa'Anna (2020)}
    ...
    \item ``Causal inference in Financial Event Studies'', Goldsmith-Pinkham and Lyu (2025)
  ...
  \item Class 15: Difference-in-differences (Part III): The Checklist and Extensions
    ...
    \item \textbf{``What’s Trending in Difference-in-Differences? A Synthesis of the Recent Econometrics Literature'' Roth, Sant’Anna,  Bilinski, and Poe (2022)}
    \item \textbf{``Difference-in-Differences Designs: A Practitioner’s Guide'', Baker, Callaway, Cunningham, Goodman-Bacon and Sant'Anna (2025)}
  ...
  \item Class 16: Instrumental Variables (Part I)
    ...
    \item \textbf{``Identification of causal effects using instrumental variables'' Angrist, Imbens and Rubin (1996)}$^{**}$
  ...
  \item Class 17: Instrumental Variables (Part II)
    ...
    \item \textbf{``Weak instruments in instrumental variables regression: Theory and practice'' Andrews, Stock and Sun (2019)}$^{**}$
  ...
  \item Class 18: Instrumental Variables (Part III)
    ...
    \item ``Tolerating defiance? Local average treatment effects without monotonicity'' de Chaisemartin
    \item ``Weak Instruments in Instrumental Variables Regression: Theory and Practice'', Andrews, Stock and Sun (2018)
    \item \textbf{``Local instrumental variables and latent variable models for identifying and bounding treatment effects'' Heckman and  Vytlacil (1999)}$^{**}$
    \end{itemize}
  \item Class 19: Bartik, Simulated, Recentered, and Granular Instruments
    \begin{itemize}
    \item \textbf{``A Practical Guide to Shift-Share Instruments'' Borusyak, Hull and Jaravel (2025, Journal of Economic Perspectives)}$^{**}$
  ...
  \item Class 20: Examiners, Leniency, and Hausman Instruments
    \begin{itemize}
    \item \textbf{``Leniency Designs: An Operator's Manual'' Goldsmith-Pinkham, Hull and Koles\'ar (2025)}$^{**}$
  ...
  \item Class 21: Regression Discontinuity I: Identification and Groundwork
    ...
    \item \textbf{``A Practical Introduction to Regression Discontinuity Designs: Foundations'', Cattaneo, Idrobo and Titiunik, (2020)}$^{**}$
  ...
  \item Class 23: Regression Discontinuity III: Extensions
    ...
    \item \textbf{``Bounds on treatment effects in regression discontinuity designs with a manipulated running variable'', Gerard, Rokkanen and Rothe (2020)}$^{**}$
...
\item \underline{Machine Learning}
  ...
  \item Class 24: Supervised Machine Learning I: Prediction
    \begin{itemize}
    \item \textbf{``Machine Learning Methods Economists Should Know About.'' Athey and Imbens (2019)}$^{**}$
  ...
  \item Class 24: (IF TIME PERMITS) Unstructured Data and Unsupervised Machine Learning
    \begin{itemize}
    \item \textbf{``Text as Data'', Kelly, Gentzkow and Taddy (2020)}$^{**}$
  ...
  \item Class 25: Supervised Machine Learning II: Heterogeneous Treatment Effects
    \begin{itemize}
    \item \textbf{``Estimation and Inference of Heterogeneous Treatment Effects using Random Forests'' Wager and Athey (2019)}$^{**}$
...
\item \underline{Miscellaneous}
  ...
  \item Class 26: Partial Identification
    ...
    \item \textbf{``Microeconometrics with Partial Identification'', Molinari (2020)}$^{**}$
    \end{itemize}
  \end{enumerate}
\end{enumerate}
...
\begin{itemize}
\item Regular attendance is not required, but is preferred. If you plan to miss more than 3 classes, please consult with me.
\end{itemize}
